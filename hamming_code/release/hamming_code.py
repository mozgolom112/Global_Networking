...
def create_words_hamming(bins_seq,flag=0): #flag=0 без ошибок flag=1 только с одиночными ошибками flag=2 только со множественными flag=3 в разброс
    ...
    len_of_word = 44
    q = False
    hamming_words = []          #будем хранить "чистые" слова
    hamming_words_error = []
    stat_err = []
        ...
        if not flag == 0:
            if flag == 1: #одиночные ошибки
                ...
                if (isError):
                    ...
                    pos_error_2 = random.randint(1,51) - 1 #т.к. массивы стартуют от 0. Следовательно от 1 до 51 и -1
                    while pos_error_2 == pos_error:
                        pos_error_2 = random.randint(1,51) - 1 #ну на всякий случай
                    error = 1 if int(hamming_word[pos_error_2]) == 0 else 0
                    hamming_word = hamming_word[:pos_error_2] + f'{error}' + hamming_word[pos_error_2+1:] 

                    stat_err.append(-2) #сохраняем -2 как флаг двойной ошибки
                else:
                    stat_err.append(-1)
                hamming_words_error.append(hamming_word)
            elif flag == 3: #все что угодно
                ...
                hamming_words_error.append(hamming_word)#сохраняем слово
        else:
            #У нас нет ошибок
            hamming_words_error.append(hamming_word)
            stat_err.append(-1)
